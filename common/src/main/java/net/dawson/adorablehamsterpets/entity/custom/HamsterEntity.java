...
public class HamsterEntity extends TameableEntity implements GeoEntity, ImplementedInventory {
    ...
    public void wakeUpFromBed(boolean isManualWakeUp) {
        ...
        this.getLinkedBedPos().ifPresent(globalPos -> {
            if (this.getWorld().getRegistryKey() == globalPos.dimension()) {
    ...
    public ActionResult interactMob(PlayerEntity player, Hand hand) {
        ...
        // --- Tag Game "Caught" Logic ---
        if (this.isPlayingTag()) {
            // Check permissions (Owner OR Stranger if allowed)
            boolean isOwner = this.isOwner(player);
            if (isOwner || Configs.AHP.allowStrangerTag) {
                if (!world.isClient) {
                    concludeTagGame(player);
                }
        ...
        if (this.isOwner(player)) {
            ...
            if (!player.isSneaking() && stack.getItem() instanceof HamsterArmorItem) {
                if (!world.isClient) {
                    ...
                    // Play equip sound
                    world.playSound(null, this.getBlockPos(), SoundEvents.ITEM_ARMOR_EQUIP_WOLF.value(), SoundCategory.NEUTRAL, 0.6f, 1.2f);
            ...
            if (this.isHoldingMouthItem() && this.isOwner(player)) {
                AdorableHamsterPets.LOGGER.trace("[InteractMob-{}] Passed 'isHoldingMouthItem' check.", this.getId());
                if (!world.isClient) {
                    ItemStack retrievedStack = this.getMouthItemStack().copy();
                    player.getInventory().offerOrDrop(this.getMouthItemStack().copy());
                    this.setMouthItemStack(ItemStack.EMPTY);
                    ...
                    this.celebrationTarget = player; // Set the player who interacted as the target
            ...
            if (this.isValid(ACCESSORY_SLOT_INDEX, stack) && !player.isSneaking()) {
                if (!world.isClient) {
                    ...
                    // Case 2: General Equip or Swap
                        ...
                        // Feedback
                        world.playSound(null, this.getBlockPos(), SoundEvents.ITEM_ARMOR_EQUIP_GENERIC.value(), SoundCategory.PLAYERS, 1.0f, 1.0f);
                        ParticleEffectsUtil.spawnParticles(
                                world,
                                new Vec3d(this.getX(), this.getY() + this.getHeight() * 0.75, this.getZ()),
            ...
            if (stack.isOf(Items.SHEARS) && !player.isSneaking()) {
                ...
                if (!armorStack.isEmpty() && armorStack.getItem() instanceof HamsterArmorItem) {
                    if (!world.isClient) {
                        ...
                        this.playSound(SoundEvents.ITEM_ARMOR_UNEQUIP_WOLF, 0.8f, 1.5f);
    ...
    */
    ...
    protected Vec3d getPassengerAttachmentPos(Entity passenger, EntityDimensions dimensions, float scaleFactor) {
        ...
        double riderAdjustY = passenger instanceof LivingEntity living
                ? HamsterSeatOffsets.physicsSeatAdjustY(living, this.getScale())
                : 0.0;
     ...
     */
    private void concludeTagGame(PlayerEntity player) {
    ...
    private void handleSlotUpdateSounds(int slot, ItemStack oldStack, ItemStack newStack) {
        ...
        if (slot == ARMOR_SLOT_INDEX) {
            if (wasEmpty && !isEmpty) {
                // Equip
                this.playSound(SoundEvents.ITEM_ARMOR_EQUIP_WOLF.value(), 0.6f, 1.2f);
            } else if (!wasEmpty && isEmpty) {
                // Unequip
                this.playSound(SoundEvents.ITEM_ARMOR_EQUIP_WOLF.value(), 0.4f, 0.8f);
            } else if (!wasEmpty && !isEmpty) {
                // Swap
                this.playSound(SoundEvents.ITEM_ARMOR_EQUIP_WOLF.value(), 0.6f, 1.2f);
            }
        ...
        if (slot == ACCESSORY_SLOT_INDEX) {
            if (wasEmpty && !isEmpty) {
                // Equip
                this.playSound(SoundEvents.ITEM_ARMOR_EQUIP_GENERIC.value(), 0.6f, 1.2f);
            } else if (!wasEmpty && isEmpty) {
                // Unequip (Lower pitch)
                this.playSound(SoundEvents.ITEM_ARMOR_EQUIP_GENERIC.value(), 0.4f, 0.8f);
            } else if (!wasEmpty && !isEmpty) {
                // Swap
                this.playSound(SoundEvents.ITEM_ARMOR_EQUIP_GENERIC.value(), 0.6f, 1.2f);
            }

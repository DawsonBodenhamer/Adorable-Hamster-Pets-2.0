...
/**
 * Owns hamster animation definitions, selection precedence, trigger registration,
 * and the three GeckoLib keyframe callbacks.
 *
 * <p>Gameplay state remains owned by {@link HamsterEntity}. Selection reads the
 * entity's existing public state surface. The keyframe callbacks intentionally
 * mutate only transient renderer effect IDs or trigger another animation.</p>
 */
public final class HamsterAnimationController {
    ...
    public static void register(HamsterEntity hamster, AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(hamster, "mainController", 3, event -> {
            ...
            HamsterEntity.DozingPhase currentDozingPhase = state.dozingPhase();
            int personality = state.personality();

            if (state.shoulderPet()) {
                ShoulderAnimationState shoulderState = ShoulderAnimationState.values()[state.shoulderStateId()];
                return switch (shoulderState) {
                    case SITTING -> {
                        if (state.cleaning()) {
                            yield event.setAndContinue(CLEANING_ANIM);
                        }
                        yield event.setAndContinue(sittingAnimation(personality));
                    }
                    case LAYING_DOWN -> switch (state.shoulderLocation()) {
            ...
            if (state.knockedOut()) return event.setAndContinue(KNOCKED_OUT_ANIM);
            if (state.sulking()) return event.setAndContinue(SULKING_ANIM);
            if (state.touchingWater() && !state.onGround()) return event.setAndContinue(SWIMMING_ANIM);
            if (state.projectileDummy() || state.renderFlying()) return event.setAndContinue(FLYING_ANIM);
            if (state.taunting()) return event.setAndContinue(TAUNTING_ANIM);
            if (state.presentingItem()) return event.setAndContinue(PRESENTING_ITEM_ANIM);
            ...
            if (activeGoalName.startsWith(HamsterSniffForOreGoal.class.getSimpleName())) {
                ...
                if (horizontalSpeedSquared > 1.0E-6) return event.setAndContinue(SEEKING_ORE_ANIM);
                return event.setAndContinue(state.oreTargetAbove()
                        ? WANTS_TO_SEEK_ORE_ABOVE_ANIM
                        : WANTS_TO_SEEK_ORE_BELOW_ANIM);
            }

            if (state.celebratingDiamond()) return event.setAndContinue(BOUNCING_ANIM);
            if (state.dancing()) return event.setAndContinue(BOUNCING_ANIM);

            if (state.tamed()) {
                switch (currentDozingPhase) {
                    case DRIFTING_OFF:
                        return event.setAndContinue(driftingOffAnimation(personality));
                    case SETTLING_INTO_SLUMBER:
                        String targetDeepSleepId = state.deepSleepAnimationId();
                        if (!targetDeepSleepId.isEmpty()) {
                            return event.setAndContinue(deepSleepAnimation(targetDeepSleepId, SITTING_POSE1_ANIM));
                        } else if (state.sitting()) {
                            return event.setAndContinue(sittingAnimation(personality));
                        }
                        break;
                    case DEEP_SLEEP:
                        String deepSleepId = state.deepSleepAnimationId();
                        return event.setAndContinue(deepSleepAnimation(deepSleepId, sittingAnimation(personality)));
                }
            }

            if (!state.tamed() && state.sleeping()) {
                String deepSleepId = state.deepSleepAnimationId();
                return event.setAndContinue(deepSleepAnimation(deepSleepId, SLEEP_POSE1_ANIM));
            }

            if (state.sitting() && !state.knockedOut()) {
                return event.setAndContinue(state.cleaning()
                        ? CLEANING_ANIM
            ...
            if (horizontalSpeedSquared > 1.0E-6) {
                if (horizontalSpeedSquared > RUN_TO_SPRINT_THRESHOLD_SQUARED) return event.setAndContinue(SPRINTING_ANIM);
                if (horizontalSpeedSquared > WALK_TO_RUN_THRESHOLD_SQUARED) return event.setAndContinue(RUNNING_ANIM);
            ...
            return event.setAndContinue(hamster.getRandom().nextBoolean() ? IDLE1_ANIM : IDLE2_ANIM);
        })
                ...
                .setParticleKeyframeHandler(event -> hamster.particleEffectId = event.getKeyframeData().getEffect())
                .setSoundKeyframeHandler(event -> hamster.soundEffectId = event.getKeyframeData().getSound())
                .setCustomInstructionKeyframeHandler(event -> {
                    if (event.getKeyframeData().getInstructions().contains("trigger_jump_anim")) {
                        hamster.triggerAnim("mainController", "quick_bounce_on_back_legs");
                    }
                }));
    }

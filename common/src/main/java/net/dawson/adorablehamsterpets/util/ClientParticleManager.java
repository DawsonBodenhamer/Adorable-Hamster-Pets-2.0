...
public class ClientParticleManager {
     ...
     */
    public void addOrUpdate(BlockPos pos, String effectId, Consumer<World> logic, BiPredicate<World, BlockPos> validityCheck) {
        String key = pos.asLong() + ":" + effectId;

        activeEmitters.compute(key, (k, current) -> {
            if (current == null) {
                // Store immutable pos for the check
                return new EmitterEntry(pos.toImmutable(), logic, validityCheck, DEFAULT_TIMEOUT_TICKS);
            }
    ...
    private static class EmitterEntry {
        final BlockPos pos;
        Consumer<World> logic;
        BiPredicate<World, BlockPos> validityCheck;
        int timeoutTicks;

        EmitterEntry(BlockPos pos, Consumer<World> logic, BiPredicate<World, BlockPos> validityCheck, int timeoutTicks) {
            this.pos = pos;
            this.logic = logic;

...
public final class HamsterCombatUtil {
    ...
    public static boolean isInAttackRange(HamsterEntity hamster, LivingEntity target) {
        Box attackBox = hamster.getBoundingBox().expand(ATTACK_BOX_EXPANSION, 0.0D, ATTACK_BOX_EXPANSION);
        return attackBox.intersects(target.getBoundingBox());
    }

    public static boolean canAttackWithOwner(HamsterEntity hamster, LivingEntity target, LivingEntity owner) {
        ...
        AdorableHamsterPets.LOGGER.trace("[canAttackWithOwner] Hamster: {}, Target: {}, Owner: {}", hamster.getName().getString(), target.getName().getString(), owner.getName().getString());
